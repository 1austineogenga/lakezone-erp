...
const AVATAR_COLORS = [
  'from-brand-red to-rose-600', 'from-slate-600 to-slate-800',
  'from-red-600 to-red-800',   'from-slate-500 to-slate-700',
  'from-rose-500 to-rose-700', 'from-gray-600 to-gray-800',
]
...
function SectionHeader({ title, icon: Icon }) {
  return (
    <div className="flex items-center gap-2 mb-3">
      <div className="p-1.5 rounded-lg bg-brand-red">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-brand-slate rounded-2xl px-6 py-5 flex flex-wrap items-center justify-between gap-4 relative overflow-hidden">
        ...
        <div className="absolute bottom-0 left-32 w-40 h-40 rounded-full bg-brand-red opacity-[0.08] translate-y-12 pointer-events-none" />
        ...
        <div className="relative z-10 flex gap-3 flex-wrap">
          ...
          <div className="bg-brand-red/40 rounded-xl px-4 py-2.5 text-center min-w-[70px]">
      ...
      {/* Workforce metrics */}
      <div>
        <SectionHeader title="Workforce" icon={UsersIcon} />
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <MetricCard label="Total Employees" value={data.total_employees}
            icon={UsersIcon} accent="border-brand-red"
            sub="All active headcount" subColor="text-brand-red" to="/hr/employees" />
          <MetricCard label="Permanent Staff" value={data.total_staff}
            icon={UserPlusIcon} accent="border-brand-slate"
            sub="Full-time employees" subColor="text-brand-slate" to="/hr/employees?type=staff" />
          <MetricCard label="Casuals" value={data.total_casuals}
            icon={UsersIcon} accent="border-gray-300"
            sub="Daily / contract workers" subColor="text-gray-500" to="/hr/employees?type=casual" />
      ...
      {/* Attendance */}
      <div>
        <SectionHeader title="Today's Attendance" icon={ClockIcon} />
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <MetricCard label="Present Today" value={data.present_today}
            icon={CheckCircleIcon} accent="border-brand-red"
            sub={`${attendancePct}% attendance rate`} subColor="text-brand-red" to="/hr/attendance" />
          <MetricCard label="Absent Today" value={data.absent_today}
            icon={UserMinusIcon} accent="border-gray-300"
            sub={data.absent_today > 0 ? 'Follow up required' : 'All accounted for'}
            subColor={data.absent_today > 0 ? 'text-red-500' : 'text-gray-500'} />
          <MetricCard label="On Leave Today" value={data.on_leave_today}
            icon={CalendarDaysIcon} accent="border-brand-slate"
            sub="Approved leave" subColor="text-brand-slate" to="/hr/leave" />
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {data.pending_leave_applications > 0 && (
            <div className="flex items-center gap-4 bg-red-50 border border-red-100 rounded-2xl px-5 py-4">
              <div className="p-2.5 bg-red-100 rounded-xl shrink-0">
                <CalendarDaysIcon className="h-5 w-5 text-brand-red" />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-bold text-brand-slate">Leave Applications</p>
              ...
              <Link to="/hr/leave" className="shrink-0 px-3 py-1.5 bg-brand-red text-white text-xs font-semibold rounded-lg hover:opacity-90 transition-opacity">
                Review
              </Link>
            </div>
          )}
          {data.pending_advances > 0 && (
            <div className="flex items-center gap-4 bg-amber-50 border border-amber-100 rounded-2xl px-5 py-4">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-bold text-brand-slate">Salary Advances</p>
                <p className="text-xs text-gray-600 mt-0.5">
                  <span className="font-semibold">{data.pending_advances}</span> advance{data.pending_advances !== 1 ? 's' : ''} pending approval
                </p>
              </div>
              <Link to="/hr/advances" className="shrink-0 px-3 py-1.5 bg-brand-slate text-white text-xs font-semibold rounded-lg hover:opacity-90 transition-opacity">
                Review
              </Link>
            </div>
          )}
