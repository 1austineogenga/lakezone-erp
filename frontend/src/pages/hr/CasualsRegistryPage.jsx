...
export default function CasualsRegistryPage() {
  ...
  const [idSearch, setIdSearch]     = useState('')   // search bar for ID/name lookup
  ...
  const { data: casuals = [], isLoading } = useQuery({
    ...
    refetchInterval: 60_000, // refresh every minute so active status stays current
  })
  ...
  // Check if past 6pm — show reset notice
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-lg font-bold text-brand-slate">Daily Casuals Register</h2>
      ...
      {showForm && (
        <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-brand-slate text-sm">{existingId ? 'Update Casual' : 'Add Casual'}</h3>
