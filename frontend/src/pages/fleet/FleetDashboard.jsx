...
export default function FleetDashboard() {
  ...
  const stats = [
    ...
    { label: 'Low Fuel (<30 L)',  val: lowFuelCount,                        icon: BeakerIcon,              color: 'text-orange-600',  bg: 'bg-orange-50',  border: 'border-l-4 border-l-orange-500' },
              ...
              {live.map(v => (
                <div key={v.id} onClick={() => navigate(`/fleet/vehicles/${v.id}`)}
                  ...
                  <div className="text-right text-xs shrink-0 w-20">
                    <p className="text-gray-600">{v.last_fuel != null ? `⛽ ${Number(v.last_fuel).toFixed(1)} ${v.fuel_sensor_unit === 'L' ? 'L' : '%'}` : '—'}</p>
