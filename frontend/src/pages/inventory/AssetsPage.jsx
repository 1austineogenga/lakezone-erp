...
const emptyAsset = {
  name: '', category: 'it_equipment', serial_number: '', make_model: '',
  purchase_date: '', purchase_value: '', current_value: '',
  condition: 'good', status: 'active', location: '', assigned_to: '', notes: '',
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
        ...
        {deptName && (
          <div className="mb-4 text-xs text-blue-700 bg-blue-50 px-3 py-2 rounded-lg flex items-center gap-1.5">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <input className={inp} value={form.serial_number} onChange={e => setForm(f => ({ ...f, serial_number: e.target.value }))} />
          </div>
          <div className="col-span-2">
            <label className="block text-xs font-medium text-gray-500 mb-1">Make / Model</label>
            <input className={inp} value={form.make_model} onChange={e => setForm(f => ({ ...f, make_model: e.target.value }))} />
          </div>
          <div>
          ...
          <div>
            ...
            <input type="number" min="0" step="any" className={inp} value={form.purchase_value}
          ...
          <div>
            ...
            <input type="number" min="0" step="any" className={inp} value={form.current_value}
              onChange={e => setForm(f => ({ ...f, current_value: e.target.value }))} />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-500 mb-1">Location</label>
            <input className={inp} value={form.location} onChange={e => setForm(f => ({ ...f, location: e.target.value }))} />
          </div>
          <div className="col-span-2">
            <label className="block text-xs font-medium text-gray-500 mb-1">Assigned To</label>
            <input className={inp} value={form.assigned_to} onChange={e => setForm(f => ({ ...f, assigned_to: e.target.value }))} />
          </div>
          <div className="col-span-2">
...
export default function AssetsPage() {
  ...
  // dept name used when creating new asset for non-viewall users
                  ...
                  return (
                    <tr key={asset.id} className="hover:bg-gray-50 cursor-pointer"
                      ...
                      <td className="px-3 py-2.5">
                        <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${cat?.color ?? 'bg-gray-100 text-gray-600'}`}>
                      ...
                      <td className="px-3 py-2.5">
                        <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${st?.color ?? 'bg-gray-100 text-gray-600'}`}>
                          {st?.label ?? asset.status}
                        </span>
                      </td>
                      <td className="px-3 py-2.5 text-gray-500">{Number(asset.purchase_value).toLocaleString()}</td>
                      <td className="px-3 py-2.5 font-medium text-gray-800">{Number(asset.current_value).toLocaleString()}</td>
                      <td className="px-3 py-2.5">
                        {canEdit && (
                          <button
                            onClick={e => { e.stopPropagation(); setEditAsset(asset); setShowModal(true) }}
